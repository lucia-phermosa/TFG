// main.cpp  (Nodo gait_duty_autoadapt + socket + marcha + trote + logger)
//
// - Marcha "duty" (ir a X) = tu código original, con tus biases/gains/geom_comp.
// - Trote libre (TR:1)     = MISMO CPG pero con frecuencia y amplitudes de trote,
//                            más zancada y más lift en todas las patas.
// - TR:0                   = se detiene TODO (trote y marcha), postura neutra.
// - XY:...                 = manda meta en X y activa la marcha “duty” de ir a X.
// - Se genera un .txt con los comandos de cada joint en:
...
// Usamos exactamente el mismo CPG que en marcha, pero con parámetros de trote.
static constexpr double TROT_FREQ_HZ       = 2.2;   // más rápido
static constexpr double TROT_DUTY_STANCE   = 0.42;  // algo más swing
static constexpr double TROT_DRIVER_AMPL   = 0.55;  // amplitud base mayor (antes ~0.38)
static constexpr double TROT_ELBOW_LIFT    = 0.48;  // más levantada de pata
static constexpr double TROT_SHOULDER_GAIN = 1.7;   // escala global extra hombro en trote
static constexpr double TROT_HIP_FOLLOW    = 0.55;  // cuánto sigue la cadera al hombro
...
public:
    ...
    t0_(rclcpp::Clock().now().seconds())
  {
    ...
    // Patas y fases (trote/marcha): FL/RR en fase; FR/RL a π → trote diagonal.
    // lift_scale: traseras un poco más (RR la que más)
    legs_ = {
...
private:
    ...
    while (cmd_parser_->readNext()) {
      ...
      switch (cmd) {
        ...
        case 17: { // TR: trot_gait ON/OFF
          ...
          trot_gait_ = new_trot;
          if (trot_gait_) {
            // Al entrar en trote libre: cancelamos metas y CPG continuo
            has_goal_ = false;
            run_gait_ = true;
  ...
  // ============================================================
  // Generador CPG genérico (marcha/trote)
  //  - freq_hz         : frecuencia
  //  - duty            : duty de stance
  //  - driver_ampl     : amplitud base del hombro
  //  - elbow_lift      : altura básica de lift (escalada por lift_scale)
  //  - shoulder_scale  : escala global extra de hombro (para trote > 1)
  //  - hip_follow_gain : cuánto sigue la cadera al hombro
  // ============================================================
                       ...
                       std::vector<double> &q_out)
  {
    ...
    for (const auto& L : legs_) {
      ...
      // Hombro (driver) – aquí ampliamos bastante la zancada
      double q_sh = BIAS_SHOULDER + dir * (driver_ampl * shoulder_scale * s);
      q_sh *= shoulder_gain;

      // Cadera: abducción + seguidor del hombro
      double q_hp = (AUX_AMPL * hip_follow_gain * q_sh
                     / std::max(1e-6, driver_ampl * shoulder_scale));
      double abd  = +BIAS_HIP_ABD;
      if (L.name == "RL" || L.name == "RR") abd = -BIAS_HIP_ABD;

      // Codo – lift potente en swing, con triangle para sincronía
      double lift = 0.0;
      if (!in_stance) {
        lift = elbow_lift * L.lift_scale * half_cosine(tau);
      }
  ...
  void logJoints(double now_s, const std::vector<double> &q) {
    ...
    log_file_ << "\n";
    // No flush en cada línea para rendimiento; el cierre del nodo vacía el buffer.
  }
  ...
  void tick() {
    ...
    generateDutyCPG(
      ...
      DRIVER_AMPL,
      ELBOW_LIFT,
      1.0,        // shoulder_scale normal
      0.35,       // hip_follow_gain original
      q
    );
    ...
    // impresión periódica por consola
    if (DO_PRINT) {
